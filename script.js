...
function generateCV() {
  ...
  reader.onloadend = function () {
    ...
    const eduInputs = document.querySelectorAll("#edu-container > div");
    const eduHTML = Array.from(eduInputs).map(div => {
      const inputs = div.querySelectorAll("input");
      return `<p><strong>${inputs[0].value}</strong> (${inputs[2].value})<br>${inputs[1].value} - Grade: ${inputs[3].value}</p>`;
    }).join('');

    const expInputs = document.querySelectorAll("#exp-container > div");
    const expHTML = Array.from(expInputs).map(div => {
    ...
    const projInputs = document.querySelectorAll("#proj-container > div");
    const projHTML = Array.from(projInputs).map(div => {
